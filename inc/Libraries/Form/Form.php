...
use GGMP\Libraries\Form\Helper;
use GGMP\Libraries\Form\Field\File;
use GGMP\Libraries\Form\Field\Iconpicker;
use GGMP\Libraries\Form\Field\Map;
use GGMP\Libraries\Form\Field\Uploader;
use GGMP\Libraries\Form\Field\Taxonomy;
use GGMP\Libraries\Form\Field\Page;
...
class Form {
	...
	public function __construct() {
		// Ajax.
		add_action( 'wp_ajax_ggmp_search_users', [ '\GGMP\Libraries\Form\Helper', 'ajax_search_users' ] );
	...
	public function render_field( $field ) {
		...
		if ( ! isset( $field['id'] ) || ! isset( $field['type'] ) ) {
			return sprintf( esc_html__( 'The field ID or field type is required.', 'ggmp' ), $field ['type'] );
		}
		...
		switch ( $field['type'] ) {
			...
			case 'html':
				...
				break;
			default:
				do_action( 'ggmp_form_render_field_' . $field['type'], $field, $this );
				//return sprintf( esc_html__( 'The field type: %s does not exist!', 'ggmp' ), $field ['type'] );
				break;
		}
	}
	...
	public function ajax_user_search( $args = [] ) {
		...
		$defaults = [
			...
			'placeholder'  => esc_html__( 'Enter username', 'ggmp' ),
			'label'        => null,
			'desc'         => null,
			'class'        => '',
			'disabled'     => false,
		...
		$args['class'] = 'ggmp-ajax-user-search ' . $args['class'];

		$output = '<span class="ggmp_user_search_wrap">';
		$output .= $this->text_field( $args );
		$output .= '<span class="ggmp_user_search_results hidden"><a class="ggmp-ajax-user-cancel" aria-label="' . esc_html__( 'Cancel',
				'ggmp' ) . '" href="#">x</a><span></span></span>';
		$output .= '</span>';
	...
	public function text_field( $args ) {
		switch ( $args['type'] ) {
			case 'text_small' :
				$class        = 'ggmp-text-small';
				$args['type'] = 'text';
				break;
			case 'text_medium' :
				$class        = 'ggmp-text-medium';
				$args['type'] = 'text';
				break;
			case 'text_number' :
				$class        = 'ggmp-text-small ggmp-text-number';
				$args['type'] = 'number';
				break;
			case 'text_url' :
				$class        = 'ggmp-text-url';
			...
			case 'text_tel' :
				...
				break;
			case 'password' :
				$class        = 'ggmp-text-password';
				$args['type'] = 'password';
				break;
			case 'hidden' :
				$class        = 'ggmp-text-hidden';
				$args['type'] = 'hidden';
				break;
			default :
				$class        = '';
		...
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'ggmp-text ' . $class . ' regular-text form-control';
		} elseif ( ! strpos( $args['class'], 'ggmp-text' ) ) {
			$args['class'] .= ' ggmp-text ' . $class . ' regular-text form-control';
		}
	...
	public function colorpicker_field( $args ) {
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'ggmp-colorpicker form-control';
		} elseif ( ! strpos( $args['class'], 'ggmp-colorpicker' ) ) {
			$args['class'] .= ' ggmp-colorpicker form-control';
		}
	...
	public function output_normal( $fields ) {
		...
		if ( function_exists( 'wp_unique_id' ) ) {
			$form_id = wp_unique_id( 'ggmp-form-' );
		} else {
			$form_id = 'ggmp-form-' . (string) ++$id_counter;
		}

		echo '<div class="js-ggmp-metabox-wrap">';
	...
	public function get_repeater_field_value( $field, $field_group, $fields ) {
		 ...
		 */
		$field_value = apply_filters( "ggmp_get_repeater_field_{$field['id']}_value", $field_value, $field, $field_group, $fields );
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( 'ggmp_get_repeater_field_value', $field_value, $field, $field_group, $fields );
	...
	public function get_repeater_field_id_value( $field, $field_group, $fields ) {
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( "ggmp_get_repeater_field_{$field['id']}_id_value", $field_value, $field, $field_group, $fields );
		 ...
		 * @param string $field_id
		 */
		$field_value = apply_filters( 'ggmp_get_repeater_field_id_value', $field_value, $field, $field_group, $fields );
	...
	public function get_repeater_field_id( $field, $fields, $default = false ) {
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( "ggmp_get_repeater_field_{$field['id']}_id", $field_id, $field, $fields, $default );
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( 'ggmp_get_repeater_field_id', $field_id, $field, $fields, $default );
	...
	public function get_repeater_field_hidden_id( $field, $fields, $default = false ) {
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( "ggmp_get_repeater_field_{$field['id']}_hidden_id", $field_id, $field, $fields, $default );
		 ...
		 * @param string $field_id
		 */
		$field_id = apply_filters( 'ggmp_get_repeater_field_hidden_id', $field_id, $field, $fields, $default );
