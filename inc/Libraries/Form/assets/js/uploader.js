...
( function( $, window ) {
    ...
    function ggmp_uploader() {
        ...
        function check_number_files( file , i   ) {
            if( is_image_file(file) ) {
                if( i+1 > ggmpJS.mfile_image ){
                    return false;
                }
            } else {
                if( i+1 > ggmpJS.mfile_other ){
                    return false;
                }
            }
            return true;
        ...
        function check_filesize (  file , i  ) {
            ...
            if( is_image_file(file) ) {
                if( file.size > ggmpJS.size_image ){
                    var myToast = $.toast({
                        heading: file.name,
                        text: ggmpJS.error_upload_size,
                        icon: 'error',
        ...
        function trigger_button_upload(){
            ...
            $(".ggmp-uploader-files").each( function(){
                handleUpload( this )
            } );
    ...
    /** @namespace wp */
    window.wp = window.wp || {};
    window.wp.ggmp_uploader = ggmp_uploader();
