...
( function ( window, document, $, opal, undefined ) {
    ...
    opal.init = function () {
        ...
        // Init tab form.
        opal.initTabForm( $( '.ggmp-form-data-tabs' ) );

        // Make File List drag/drop sortable:
        opal.makeListSortable();

        // Init time/date/color pickers
        opal.initPickers( $metabox.find( 'input[type="text"].ggmp-colorpicker' ),
            $metabox.find( 'input[type="text"].ggmp-datepicker' ) );

        $metabox
            .on( 'change', '.ggmp_upload_file', function () {
                opal.media.field = $( this ).attr( 'id' );
                $id( opal.media.field + '_id' ).val( '' );
            } )
            // Media/file management
            .on( 'click', '.ggmp-upload-button', opal.handleMedia )
            .on( 'click', '.ggmp-attach-list li, .ggmp-media-status .img-status img, .ggmp-media-status' +
                ' .file-status >' +
                ' span', opal.handleFileClick )
            .on( 'click', '.ggmp-remove-file-button', opal.handleRemoveMedia );

        opal.mapping();
        /////
        ///
        opal.groupFields( $(".ggmp-group-field-wrap", $metabox) );
        ///
        $( '.ggmp-select' ).select2();
        ///
        ...
        $metabox.find( '.ggmp-iconpicker' ).each( function () {
            $( this ).fontIconPicker();
        } );
    };
    ...
    opal.initTabForm = function ( $tabs ) {
        $( $tabs ).each( function () {
            ...
            $( 'a', $tab ).click( function () {
                $( '.ggmp_options_panel', $parent ).hide();
    ...
    opal.makeListSortable = function() {
        var $filelist = opal.metabox().find( '.ggmp-media-status.ggmp-attach-list' );
        if ( $filelist.length ) {
    ...
    opal.datePickerSetupOpts = function ( fieldOpts, options, method ) {
        ...
        options.beforeShow = function ( input, inst ) {
            ...
            $id( 'ui-datepicker-div' ).addClass( 'ggmp-element' );
        ...
        options.onClose = function ( dateText, inst ) {
            ...
            var $picker = $id( 'ui-datepicker-div' ).removeClass( 'ggmp-element' ).hide();
    ...
    opal.handleMedia = function ( evt ) {
        ...
        var $el = $( this );
        opal.attach_id = !$el.hasClass( 'ggmp-upload-list' ) ? $el.closest( '.ggmp-field-wrap' )
                                                                     .find( '.ggmp-upload-file-id' )
                                                                     .val() : false;
        // Clean up default 0 value
        opal.attach_id = '0' !== opal.attach_id ? opal.attach_id : false;

        opal._handleMedia( $el.prev( 'input.ggmp-upload-file' ).attr( 'id' ),
            $el.hasClass( 'ggmp-upload-list' ) );
    };
    ...
    opal.handleFileClick = function ( evt ) {
        ...
        evt.preventDefault();
        var $el = $( this );
        var $td = $el.closest( '.ggmp-field-wrap' );
        var isList = $td.find( '.ggmp-upload-button' ).hasClass( 'ggmp-upload-list' );
        opal.attach_id = isList ? $el.find( 'input[type="hidden"]' ).data( 'id' ) : $td.find(
            '.ggmp-upload-file-id' ).val();

        if ( opal.attach_id ) {
            opal._handleMedia( $td.find( 'input.ggmp-upload-file' ).attr( 'id' ), isList, opal.attach_id );
        }
    };
        ...
        handlers.list = function ( selection, returnIt ) {
            ...
            if ( !handlers.list.templates ) {
                handlers.list.templates = {
                    image: wp.template( 'ggmp-list-image' ),
                    file: wp.template( 'ggmp-list-file' ),
                };
            }
            ...
            if ( !returnIt ) {
                // Append each item from our fileGroup array to .ggmp-media-status
                media.$field.siblings( '.ggmp-media-status' ).append( fileGroup );
            } else {
                return fileGroup;
            }
        ...
        handlers.single = function ( selection ) {
            if ( !handlers.single.templates ) {
                handlers.single.templates = {
                    image: wp.template( 'ggmp-single-image' ),
                    file: wp.template( 'ggmp-single-file' ),
                };
            }
            ...
            media.$field.siblings( '.ggmp-media-status' ).slideDown().html( attachmentHtml );
        };
    ...
    opal.handleRemoveMedia = function ( evt ) {
        evt.preventDefault();
        var $this = $( this );
        if ( $this.is( '.ggmp-attach-list .ggmp-remove-file-button' ) ) {
            $this.parents( '.ggmp-media-item' ).remove();
            return false;
        }
        ...
        $this.parents( '.ggmp-media-status' ).html( '' );
    ...
    opal.loadAutosugesstion = function () {
       ...
       load_select2_member( '#post_author_override', 'ggmp_search_users' );
    };
    opal.groupFields = function ( $group_containers ) {
        ...
        $group_containers.each( function() {
            var $_this = $( this );
            var $template = $( ".ggmp-template", this );
            ...
            $(".add-repeat-group-btn", $_this ).on( "click" , function() {
                ...
                changeIndexes( $_this );
                // $( this ).closest( 'table' ).find('tbody tr:last-child select.ggmp-select').select2();
            } );
            ...
            $(  $_this ).on( 'click', ".opal-row-repeater button" , function() {
             ...
             $(  $_this ).on( 'click', ".opal-row-repeater .ggmp-remove" , function() {
                var group = $( this ).parent().parent().parent().parent();
                $( group ).remove();
                changeIndexes( $_this );
            } );
    ...
    opal.mapping = function() {
        ...
        var condition_cache = '';
        $('#ggmp-add-new-condition').on('click', function (e) {
            e.preventDefault();
            var $filter_body = $('#ggmp-table-filter tbody');
            ...
            if (condition_cache) {
                ...
                $.ajax({
                    ...
                    method: 'POST',
                    data: {
                        action: 'ggmp_add_new_filter_condition'
                    }
        ...
        $(document).on('click', '.ggmp-del-condition', function (e) {
            e.preventDefault();
        ...
        function condition_sortable_init() {
            var $table_body = $('.ggmp-table-filter tbody');
        ...
        function no_conditions() {
            var no_conditions = $('#ggmp-table-filter tbody tr:not(.ggmp-no-conditions)').length;

            if (no_conditions >= 1) {
                $('.ggmp-no-conditions').hide();
            } else {
                $('.ggmp-no-conditions').show();
            }
        }

        function select2_mapping_init() {
            $( '.ggmp-table-filter select' ).select2();
        }
    }
    ...
    opal.metabox = function () {
        ...
        opal.$metabox = $( '.js-ggmp-metabox-wrap' );
        return opal.$metabox;
    };
