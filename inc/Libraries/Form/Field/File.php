...
class File {
	...
	public function __construct( $args, Form $form, $type = 'file' ) {
		$classes = [
			'ggmp-upload-file',
			( 'file_list' === $type ) ? 'ggmp-upload-list' : '',
			'regular-text',
			'form-control',
		];
		...
		$defaults = [
			...
			'name'            => '',
			'button_text'     => isset( $args['button_text'] ) ? esc_html( $args['button_text'] ) : esc_html__( 'Add or Upload File', 'ggmp' ),
			'description'     => null,
	...
	public function render_file() {
		...
		$output = '<div class="ggmp-field-wrap ggmp-file-field-wrap form-group" id="' . sanitize_key( $this->form->form_id . $this->args['id'] ) . '-wrap" >';

		$output .= '<label class="ggmp-label" for="' . sanitize_key( $this->form->form_id . $this->args['id'] ) . '">' . esc_html( $this->args['name'] ) . '</label>';
		$output .= '<div class="ggmp-field-main">';
		...
		$output .= '<input class="ggmp-upload-button button-secondary" type="button" value="' . $this->args['button_text'] . '">';
		$output .= '<input type="hidden" class="ggmp-upload-file-id" name="' . esc_attr( $this->args['hidden_id'] ) . '" id="' . sanitize_key( $this->args['hidden_id'] ) . '" value="' . esc_attr(
				$this->args['id_value']
			) . '">';

		if ( ! empty( $this->args['description'] ) ) {
			$output .= '<p class="ggmp-description">' . $this->args['description'] . '</p>';
		}

		$output .= '<div id="' . esc_attr( $this->args['id'] ) . '-status" class="ggmp-media-status">';
	...
	public function render_file_list() {
		...
		$output = '<div class="ggmp-field-wrap ggmp-file-list-field-wrap form-group" id="' . sanitize_key( $this->form->form_id . $this->args['id'] ) . '-wrap" >';
		...
		$output .= '<input class="ggmp-upload-button ggmp-upload-list button-secondary" type="button" value="' . $this->args['button_text'] . '">';
		$output .= '<input type="hidden" class="ggmp-upload-file-id" name="' . esc_attr( $this->args['hidden_id'] ) . '" id="' . sanitize_key( $this->args['hidden_id'] ) . '" value="' . esc_attr(
				$this->args['id_value']
			) . '">';
		...
		$output .= '<ul id="' . esc_attr( $this->args['id'] ) . '-status" class="ggmp-media-status ggmp-attach-list">';
	...
	public function get_file_preview_output() {
		...
		if ( $this->args['id_value'] ) {
			$image = wp_get_attachment_image( $this->args['id_value'], $this->args['preview_size'], null, [
				'class' => 'ggmp-file-field-image',
			] );
		} else {
			$image = '<img style="max-width: 50px; width: 100%;" src="' . $this->args['value'] . '" class="ggmp-file-field-image" alt="" />';
		}
	...
	public function img_status_output( $args ) {
		return sprintf( '<%1$s class="img-status ggmp-media-item">%2$s<p class="ggmp-remove-wrapper"><a href="#" class="ggmp-remove-file-button"%3$s>%4$s</a></p>%5$s</%1$s>',
			$args['tag'],
			$args['image'],
			isset( $args['cached_id'] ) ? ' rel="' . $args['cached_id'] . '"' : '',
			esc_html__( 'Remove Image', 'ggmp' ),
	...
	public function file_status_output( $args ) {
		return sprintf( '<%1$s class="file-status ggmp-media-item"><span>%2$s <strong>%3$s</strong></span>&nbsp;&nbsp; (<a href="%4$s" target="_blank" rel="external">%5$s</a> / <a href="#" class="ggmp-remove-file-button"%6$s>%7$s</a>)%8$s</%1$s>',
			$args['tag'],
			esc_html__( 'File:', 'ggmp' ),
			$this->get_file_name_from_path( $args['value'] ),
			$args['value'],
			esc_html__( 'Download', 'ggmp' ),
			isset( $args['cached_id'] ) ? ' rel="' . $args['cached_id'] . '"' : '',
			esc_html__( 'Remove', 'ggmp' ),
			isset( $args['id_input'] ) ? $args['id_input'] : ''
		);
	}
	...
	public function is_valid_img_ext( $file, $blah = false ) {
		...
		$is_valid_types = apply_filters( 'ggmp_valid_img_types', $valid_types );
	...
	public static function output_js_underscore_templates() {
		?>
        <script type="text/html" id="tmpl-ggmp-single-image">
            <div class="img-status ggmp-media-item">
                <img width="{{ data.sizeWidth }}" height="{{ data.sizeHeight }}" src="{{ data.sizeUrl }}" class="ggmp-file-field-image" alt="{{ data.filename }}" title="{{ data.filename }}"/>
                <p><a href="#" class="ggmp-remove-file-button" rel="{{ data.mediaField }}">{{ data.stringRemoveImage }}</a></p>
            </div>
        </script>
        <script type="text/html" id="tmpl-ggmp-single-file">
            <div class="file-status ggmp-media-item">
                <span>{{ data.stringFile }} <strong>{{ data.filename }}</strong></span>&nbsp;&nbsp; (<a href="{{ data.url }}" target="_blank" rel="external">{{ data.stringDownload }}</a> / <a
                        href="#" class="ggmp-remove-file-button" rel="{{ data.mediaField }}">{{ data.stringRemoveFile }}</a>)
            </div>
        </script>
        <script type="text/html" id="tmpl-ggmp-list-image">
            <li class="img-status ggmp-media-item">
                <img width="{{ data.sizeWidth }}" height="{{ data.sizeHeight }}" src="{{ data.sizeUrl }}" class="ggmp-file_list-field-image" alt="{{ data.filename }}">
                <p><a href="#" class="ggmp-remove-file-button" rel="{{ data.mediaField }}[{{ data.id }}]">{{ data.stringRemoveImage }}</a></p>
                <input type="hidden" id="filelist-{{ data.id }}" data-id="{{ data.id }}" name="{{ data.mediaFieldName }}[{{ data.id }}]" value="{{ data.url }}">
            </li>
        </script>
        <script type="text/html" id="tmpl-ggmp-list-file">
            <li class="file-status ggmp-media-item">
                <span>{{ data.stringFile }} <strong>{{ data.filename }}</strong></span>&nbsp;&nbsp; (<a href="{{ data.url }}" target="_blank" rel="external">{{ data.stringDownload }}</a> / <a
                        href="#" class="ggmp-remove-file-button" rel="{{ data.mediaField }}[{{ data.id }}]">{{ data.stringRemoveFile }}</a>)
                <input type="hidden" id="filelist-{{ data.id }}" data-id="{{ data.id }}" name="{{ data.mediaFieldName }}[{{ data.id }}]" value="{{ data.url }}">
            </li>
        </script>
		<?php
