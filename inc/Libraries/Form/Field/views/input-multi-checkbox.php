...
$output = '<div class="ggmp-field-wrap ggmp-multicheckbox-wrap form-group" id="' . sanitize_key( $this->form_id . $args['id'] ) . '-wrap" >';
if ( $args['show_label'] ) {
	$output .= '<label class="ggmp-label" for="' . esc_attr( sanitize_key( str_replace( '-', '_', $this->form_id . $args['id'] ) ) ) . '">' . esc_html( $args['name'] ) . '</label>';
}
...
foreach ( $args['options'] as $key => $option_name ) {
	...
	$output .= '<label class="ggmp-option-label" for="' . $_id . '">' . esc_html( $option_name ) . '</label>';
	$output .= '</span>';
}
