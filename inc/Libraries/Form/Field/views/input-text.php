...
$defaults = [
	...
	'description'  => null,
	'placeholder'  => '',
	'class'        => 'ggmp-text regular-text form-control',
	'wrap_class'   => '',
	'disabled'     => false,
	'autocomplete' => 'off',
	'data'         => [],
...
if ( 'hidden' !== $args['type'] ) {
	$output .= '<div class="ggmp-field-wrap ggmp-text-field-wrap form-group ' . $wrap_class . '" id="' . sanitize_key( $this->form_id . $args['id'] ) . '-wrap">';

	if ( $args['name'] && $this->show_label ) {
		$output .= '<label class="ggmp-label" for="' . sanitize_key( $this->form_id . $args['id'] ) . '">' . esc_html( $args['name'] ) . $required_label . '</label>';
	}
