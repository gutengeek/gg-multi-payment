...
class Taxonomy {
	...
	public function __construct( $args, Form $form, $type = 'taxonomy_select' ) {
		...
		$defaults = [
			...
			'multiple'          => false,
			'select_atts'       => false,
			'show_option_all'   => esc_html__( 'All', 'ggmp' ),
			'show_option_none'  => esc_html__( 'None', 'ggmp' ),
	...
	private function render_check_item( $args, $term ) {
		...
		$output .= '<label class="ggmp-option-label" for="' . $_id . '">' . esc_html( $term->name ) . '</label>';
		$output .= '</span>';
	...
	public function render_custom_style( $ismulti = true ) {
		...
		$output = '<div class="ggmp-field-wrap ggmp-taxonomy-select-wrap form-group" id="' . sanitize_key( $this->form->form_id . $args['id'] ) . '-wrap" >';
		if ( $args['show_label'] ) {
	...
	public function render_taxonomy_select() {
		...
		if ( ! empty( $args['description'] ) ) {
			$output .= '<p class="ggmp-description">' . esc_html( $args['description'] ) . '</p>';
		}
	...
	public function render_taxonomy_multicheck() {
		...
		$output = '<div class="ggmp-field-wrap ggmp-taxonomy-multicheck-wrap form-group" id="' . sanitize_key( $this->form->form_id . $args['id'] ) . '-wrap" >';
		if ( $args['show_label'] ) {
			$output .= '<label class="ggmp-label" for="' . esc_attr( sanitize_key( str_replace( '-', '_', $this->form->form_id . $args['id'] ) ) ) . '">' . esc_html( $args['name'] ) . '</label>';
		}
		...
		if ( isset( $args['select_all_button'] ) && $args['select_all_button'] ) {
			$output .= '<div class="ggmp-multicheck-action"><span class="button-secondary ggmp-multicheck-toggle">' . esc_html__( 'Select / Deselect All', 'ggmp' ) . '</span></div>';
		}
	...
	public function render_taxonomy_radio() {
		...
		$output = '<div class="ggmp-field-wrap ggmp-taxonomy-radio-wrap form-group" id="' . sanitize_key( $this->form->form_id . $args['id'] ) . '-wrap" >';
		if ( $args['show_label'] ) {
			$output .= '<label class="ggmp-label" for="' . esc_attr( sanitize_key( str_replace( '-', '_', $this->form->form_id . $args['id'] ) ) ) . '">' . esc_html( $args['name'] ) . '</label>';
		}
	...
	protected function no_terms_result( $error, $tag = 'li' ) {
		if ( is_wp_error( $error ) ) {
			...
			$message = esc_html__( 'No terms', 'ggmp' );
			$data    = '';
		}
