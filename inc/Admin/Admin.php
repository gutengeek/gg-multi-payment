...
class Admin {
	...
	public function add_plugin_admin_menu() {
		add_menu_page(
			apply_filters( $this->plugin_name . '-dashboard-page-title', esc_html__( 'Payment Routing', 'ggmp' ) ),
			apply_filters( $this->plugin_name . '-dashboard-menu-title', esc_html__( 'Payment Routing', 'ggmp' ) ),
			'manage_options',
			$this->plugin_name,
			[ $this, 'page_options' ],
			'dashicons-groups',
		...
		add_submenu_page(
			$this->plugin_name,
			apply_filters( $this->plugin_name . '-settings-page-title', esc_html__( 'Settings', 'ggmp' ) ),
			apply_filters( $this->plugin_name . '-settings-menu-title', esc_html__( 'Settings', 'ggmp' ) ),
			'manage_options',
			$this->plugin_name,
			[ $this, 'page_options' ]
		);
	...
	private function load_object_settings() {
		if ( empty( $this->settings_objs ) ) {
			$matching = apply_filters( 'ggmp_load_settings', [
	...
	public function update_settings() {
		if ( isset( $_POST['save_page_options'] ) ) {
			...
			if ( isset( $objects[ $tab ] ) ) {
				...
				$objects[ $tab ]->save_settings_options( $settings, 'ggmp_settings' );
			}
		}
	}
	...
	public function page_options() {
		...
		echo '<div class="ggmp-settings-page">';
		...
		$form->setup( 'page_options', 'ggmp_settings' );
		...
		echo '<button class="ggmp-btn ggmp-btn-submit" name="save_page_options" value="savedata" type="submit">' . esc_html__( 'Save' ) . '</button>';
		echo '</form>';
