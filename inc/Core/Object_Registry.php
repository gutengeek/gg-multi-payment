...
if ( ! trait_exists( 'Object_Registry' ) ) {
	...
	trait Object_Registry {
		...
		public static function set( $key, $value ) {
			if ( ! is_string( $key ) ) {
				trigger_error( esc_html__( 'Key passed to `set` method must be key', GGMP::PLUGIN_ID ), E_USER_ERROR ); // @codingStandardsIgnoreLine.
			}
		...
		public static function get( $key ) {
			if ( ! is_string( $key ) ) {
				trigger_error( esc_html__( 'Key passed to `get` method must be key', GGMP::PLUGIN_ID ), E_USER_ERROR ); // @codingStandardsIgnoreLine.
			}
