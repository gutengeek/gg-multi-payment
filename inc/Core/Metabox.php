...
abstract class Metabox {
	...
	public function __construct() {
		...
		$this->metabox_label = esc_html__( 'Options', 'ggmp' );

		add_action( 'save_post_ggmps', [ $this, 'save' ], 10, 2 );
	}
			...
			foreach ( $form_meta_keys as $form_meta_key ) {
				...
				if ( 'taxonomy_select' === $setting_field['type'] ) {
					if ( isset( $setting_field['multiple'] ) && $setting_field['multiple'] ) {
						$_POST[ $form_meta_key ] = isset( $_POST[ $form_meta_key ] ) ? ggmp_clean( $_POST[ $form_meta_key ] ) : [];
					} else {
						$_POST[ $form_meta_key ] = isset( $_POST[ $form_meta_key ] ) ? ggmp_clean( $_POST[ $form_meta_key ] ) : '';
					}
				}
	...
	public function save_settings_options( $options, $option_name ) {
		...
		if ( $update_options ) {
			...
			do_action( 'ggmp_after_save_settings', $update_options, $old_options );
		}
	}
