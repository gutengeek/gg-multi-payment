...
class Search {
	...
	public static function search_products( $post ) {
		...
		$id              = absint( $post['id'] );
		$ids             = ggmp_sanitize_ids( $post['ids'] );
		$excluded_ids    = [ $id ];
		$products_exists = explode( ',', $ids );
		$limit           = ggmp_get_option( 'search_limit', '5' );
	...
	public static function search_by_sku( $query ) {
		if ( $query->is_search && isset( $query->query['is_ggmp'] ) ) {
			global $wpdb;
			$sku = $query->query['s'];
